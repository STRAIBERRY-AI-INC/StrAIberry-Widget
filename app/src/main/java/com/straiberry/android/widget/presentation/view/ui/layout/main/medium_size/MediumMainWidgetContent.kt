package com.straiberry.android.widget.presentation.view.ui.layout.main.medium_size
...
val glanceBodyText14 = TextStyle(
    fontSize = 21.sp,
    fontWeight = FontWeight.Bold
)

@Composable
fun MediumMainWidgetContent(
...
fun LastBrushStatus(context: Context, brushCount: Int, lastBrushTime: String, remindBrush: String) {
    Box(modifier = GlanceModifier.padding(start = 43.dp)) {

        AndroidRemoteViews(remoteViews = BrushingCountViewMediumSize(context).apply {
            setTextViewText(
...
fun MainTitle(context: Context) {
    ...
    Row(modifier = GlanceModifier.padding(start = 22.dp, top = 19.dp), verticalAlignment = Alignment.CenterVertically) {
        Image(
...
fun BrushingButton(context: Context) {
    Box(modifier = GlanceModifier.padding(start = 52.dp), contentAlignment = Alignment.Center) {
        Image(
        ...
        Image(
            ...
            modifier = GlanceModifier.size(99.dp, 81.dp)
        )
    }
}

class BrushingCountViewMediumSize(context: Context) :
    RemoteViews(context.packageName, R.layout.current_day_brushing_count_medium_siz)
